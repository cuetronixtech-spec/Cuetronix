...
function StatusBadge({ status, isActive }: { status: string; isActive: boolean }) {
  ...
  const label = { active: "Active", trial: "Trial", past_due: "Past Due", canceled: "Canceled", inactive: "Inactive" }[key] ?? key;
  return (
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {loading ? "Loading…" : `${tenants.length} clubs total`}
          </p>
        </div>
              ...
              {loading ? (
                ...
                filtered.map((t) => (
                  <TableRow key={t.id} className="border-border hover:bg-muted/30">
                    ...
                    <TableCell>
                      <StatusBadge status={t.subscription_status} isActive={t.is_active} />
                    </TableCell>
                    <TableCell className="text-sm text-muted-foreground">
                      {t.plan_name || "—"}
                    </TableCell>
                    ...
                    <TableCell className="text-sm text-muted-foreground">
                      {t.trial_ends_at
                        ? format(new Date(t.trial_ends_at), "dd MMM yyyy")
                        : "—"}
                    </TableCell>
                    <TableCell className="text-sm text-muted-foreground">
