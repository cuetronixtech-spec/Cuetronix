...
CREATE POLICY "stations_delete" ON stations FOR DELETE
  USING (tenant_id = auth.tenant_id() AND auth.is_admin_or_manager());
...
CREATE POLICY "staff_profiles_update" ON staff_profiles FOR UPDATE
  USING (tenant_id = auth.tenant_id() AND (auth.is_admin() OR user_id = auth.uid()));
...
CREATE POLICY "staff_work_schedules_insert" ON staff_work_schedules FOR INSERT
  WITH CHECK (tenant_id = auth.tenant_id() AND auth.is_admin_or_manager());
...
CREATE POLICY "staff_payroll_select" ON staff_payroll FOR SELECT
  USING (
    ...
    AND (auth.is_admin_or_manager() OR user_id = auth.uid())
  );
...
CREATE POLICY "notifications_update" ON notifications FOR UPDATE
  USING (tenant_id = auth.tenant_id() AND (user_id IS NULL OR user_id = auth.uid()));
...
CREATE POLICY "tenants_select" ON tenants FOR SELECT
  USING (id = auth.tenant_id() AND is_active = TRUE);
